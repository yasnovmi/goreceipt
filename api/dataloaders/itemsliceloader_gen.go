...
func (l *ItemSliceLoader) Load(key int) ([]*api.Item, error) {
...
func (l *ItemSliceLoader) LoadThunk(key int) func() ([]*api.Item, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*api.Item, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ItemSliceLoader) LoadAll(keys []int) ([][]*api.Item, []error) {
	results := make([]func() ([]*api.Item, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	items := make([][]*api.Item, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ItemSliceLoader) LoadAllThunk(keys []int) func() ([][]*api.Item, []error) {
	results := make([]func() ([]*api.Item, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ItemSliceLoader) Prime(key int, value []*api.Item) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*api.Item, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *ItemSliceLoader) unsafeSet(key int, value []*api.Item) {
	if l.cache == nil {
		l.cache = map[int][]*api.Item{}
	}
	l.cache[key] = value
}
